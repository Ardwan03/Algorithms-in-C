...
void free_queue(queue *kö) { // Funktion som frigör allt minne kopplat till kön.
    free(kö->array); // Frigör arrayen som innehåller elementen.
    free(kö); // Frigör kön.
}
...
void expandera_queue(queue *kö) { // Funktion som gör arrayen större när kön är full.
    ...
    for (int i = 0; i < kö->antalElement; i++) // Loopar över alla element som finns i kön.
        nyArray[i] = kö->array[(kö->first + i) % kö->arrayKapacitet]; // Kopiera i korrekt FIFO-ordning.
...
int main() {
    ...
    queue *kö = create_queue(2); // Skapar en kö med liten kapacitet för att trigga expansion.

    enqueue(kö, 10); // Lägg in 10.
    enqueue(kö, 20); // Lägg in 20, kön är full.
    enqueue(kö, 30); // Lägg in 30, ska anropa expansion.
