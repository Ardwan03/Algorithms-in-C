...
{
    int first = 0; // första möjliga index
    int last  = length - 1; // sista möjliga index
    ...
    while (true)
    {
        ...
        {
            first = index + 1; // sök i högra halvan
            continue;
        }

        if (array[index] > key && index > first)
        {
            last = index - 1; // sök i vänstra halvan
            continue;
        }

        // Om intervallet inte kan krympas mer
        return false; // talet finns inte
    }
}

int main()
{
    int length = 1000000; // storlek på arrayen
    ...
    binary_search(array, length, key); // anropa binärsökning
